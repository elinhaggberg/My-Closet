// Storage-based image sync for Cloud Backup -- uploads a card's actual
// image bytes once to this project's private Supabase Storage bucket via
// the backup-image Edge Function, and syncs a small
// "storage:<store>:<recordId>" reference in the record's place instead of
// inlining the full image. A pulled reference resolves lazily -- see
...
// periodic sync) never has to download every image up front, only the ones
...
// nothing about it changed. Not a content hash: a save is the only moment
// an image could have changed, and saveCard already bumps updatedAt on
// every save, so that's a good enough signal at personal-register scale.
const SYNCED_VERSIONS_KEY = "mc_image_synced_versions_v1";
...
// Upload side, called from cloudBackup.js's pushAll. A device-local
// "idb:<id>" image is uploaded (once per change, see alreadySynced above)
// to the project's private Storage bucket, and the record synced in its
// place carries the small storage: reference instead. Anything else (a
// remote unfurled image URL, or no image at all) passes through unchanged.
//
...
// so every existing resolveImageSrc call site (pin cards, board grids...)
// picks up storage: references automatically, no call-site changes needed.
// Fetches a short-lived signed URL from the Edge Function, downloads the
// bytes once, and caches them into this device's own image store --
// patchRecordImage (injected per app, since each app's records live in a
// differently-shaped local store) then rewrites the record's image field
// from the storage: reference to a plain idb: one, so every subsequent
// open resolves instantly with no network round trip at all, exactly like
// a record whose image was always local to begin with.
