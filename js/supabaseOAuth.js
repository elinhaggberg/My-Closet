...
const API_CONFIG_KEY = "mc_supabase_api_config_v1";
const WIZARD_STEP_KEY = "mc_cloud_wizard_step_v1";
...
// including unrelated ones (e.g. a sibling app's). This is how the user
// explicitly says which one is My Closet's, stored separately from the
// connection itself so switching projects doesn't need a reconnect.
...
// Runtime Cloud Backup API config (project URL + publishable key) that
// cloudBackup.js/cloudImageSync.js call the deployed backup-sync/
// backup-image functions with -- distinct from the OAuth access token
// above, which is only ever used for Management API install calls, never
// for normal day-to-day sync. Written once by js/cloudSyncInstall.js right
