...
-- store used to be constrained to just this app's own record type
-- ("cards") -- that broke any other Make It Local app trying to share
-- this same project, since its own store name (e.g. "profiles" or "books")
-- would get rejected by this table alone. Each app already filters
-- incoming records to its own known stores client-side (see
-- cloudBackup.js's SYNCABLE_STORES), so this constraint was never doing
-- anything a client-side check wasn't already doing -- dropped here
